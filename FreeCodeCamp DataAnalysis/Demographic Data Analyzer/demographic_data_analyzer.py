...
def calculate_demographic_data(print_data=True):
    # Read data from file
    df = pd.read_csv('FreeCodeCamp DataAnalysis/Demographic Data Analyzer/adult_data.csv')
    ...
    mask = df['sex'] == 'Male'
    average_age_men = df['age'][mask].mean().__round__(1)

    # What is the percentage of people who have a Bachelor's degree?
    #percentage_bachelors = df[df['education'] == 'Bachelors'].value_counts()
    percentage_bachelors = df['education'][df['education'] == 'Bachelors'].shape[0]/df.shape[0]*100
    ...
    higher_education = df['education'][(df['education'] == 'Bachelors') | (df['education'] == 'Masters') | (df['education'] == 'Doctorate')].shape[0]
    lower_education = df['education'][~((df['education'] == 'Bachelors') | (df['education'] == 'Masters') | (df['education'] == 'Doctorate'))].shape[0]

    # percentage with salary >50K
    higher_education_rich = round(df['education'][(df['salary'] == '>50K') & ((df['education'] == 'Bachelors') | (df['education'] == 'Masters') | (df['education'] == 'Doctorate'))].shape[0]/
                                  higher_education*100, 1)
    lower_education_rich = round(df[(df['salary'] == '>50K') & ((df['education'] != 'Bachelors') & (df['education'] != 'Masters') & (df['education'] != 'Doctorate'))].shape[0]/
    ...
    rich_percentage = round(df['hours-per-week'][(df['salary'] == '>50K') & (df['hours-per-week'] == min_work_hours)].shape[0]/num_min_workers*100, 1)

    # What country has the highest percentage of people that earn >50K?
    teste = (df['native-country'][df['salary'] == '>50K'].value_counts()/df['native-country'].value_counts()).idxmax()
